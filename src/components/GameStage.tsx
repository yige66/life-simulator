...
export default function GameStage({ character, onUpdateStats, onGameEnd }: Props) {
  ...
  const STAT_META: Record<string, { keys: string[] }> = {
    ...
    体力: { keys: ['体力', '力量', '身体', '强壮', '坚韧', '战斗', '伤势', '奔跑', '疲惫', '虚弱', '累倒', '透支', '力气', '体魄', '体术', '肌肉'] },
...
7. ★★★ **consequence必须硬性提及属性名和变化方向**：
   每个consequence必须明确写出"XX属性上升/增强了"或"XX属性下降/减弱了"这样的句式。
   不允许省略——如果某个属性的effects不为零，consequence中必须出现对该属性的描述。
   例如："你的推理让智力上升了"，"伤势导致体力减弱"，"你成功吸引了众人，魅力提升"，"运气不幸地跌到了谷底"。
   注意：体力是角色身体素质和力量的综合体现，不是消耗性道具。描述体力变化时不要说"体力用完了"，而应说"体力减弱/体力增强"。

返回严格JSON：
{
  "event": "对玩家动作的世界回应（80-120字），描述动作导致的结果和当前场面",
  "options": [{"text": "选项", "effects": {"智力": 1}, "consequence": "必须用日常语气明确说明该选项为何导致属性上下变化"}],
  "actionEffects": {"智力": -2},
  "consequence": "动作的后果简述（40-60字），必须提及受影响的属性名和变化方向",
  "chapterEnd": false
}
...
4. 不能涉及政治话题/政治人物/政治隐喻
5. 不能涉及色情/暴力/歧视，保持在PG-13
6. ★★★ **consequence必须硬性提及属性名和变化方向**：
   每个consequence必须明确写出"XX属性上升/增强了"或"XX属性下降/减弱了"这样的句式。
   不允许省略——如果某个属性的effects不为零，consequence中必须出现对该属性的描述。
   例如："推理能力飞跃让智力上升了"，"伤势加重使体力减弱"，"你吸引了众人魅力提升"，"运气不幸地跌到了谷底"。
   注意：体力是角色身体素质和力量的综合体现，不是消耗性道具。描述体力变化时不要说"体力用完了"，而应说"体力减弱/体力增强"。
...
返回严格JSON：
  ...
  "options": [{"text": "选项", "effects": {"智力": 1}, "consequence": "必须用日常语气明确说明该选项为何导致属性上下变化"}],
  "actionEffects": {"智力": -2},
  "consequence": "选择后的直接后果（40-60字），必须提及受影响的属性名和变化方向",
  "chapterEnd": false
}
